...
function Necrosis_Initialize()
	...
	-- On initialise ! Si le joueur n'est pas D�moniste, on cache Necrosis (chuuuut !)
	-- On indique aussi que N�crosis est initialis� maintenant
	if UnitClass("player") ~= NECROSIS_UNIT_WARLOCK then
		...
		-- Si le d�moniste a une arme une main d'�quip�e, on lui �quipe le premier objet li� main gauche
...
function Necrosis_SlashHandler(arg1)
	-- Blah blah blah, le joueur est-il bien un D�moniste ? On finira par le savoir !
	if UnitClass("player") ~= NECROSIS_UNIT_WARLOCK then
		return;
	end
