...
-- D�tection des initialisations du mod
local NecrosisRL = true;

-- Initialisation des variables utilis�es par Necrosis pour la gestion des sorts lanc�s
...
-- Initialisation des tableaux g�rant les Timers
...
-- Variables utilis�es pour la gestion des transes de l'ombre
local ShadowTrance = false;
local AntiFearInUse = false;
local ShadowTranceID = -1;

-- Variables utilis�es pour la gestion des fragments d'�me
...
-- Variables utilis�es pour la gestion des composants d'invocation
-- (principalement comptage)
local InfernalStone = 0;
local DemoniacStone = 0;


-- Variables utilis�es pour la gestion des boutons d'invocation et d'utilisation des pierres
...
-- Variables g�rant la possibilit� ou l'impossibilit� d'utiliser un timer de rez
local SoulstoneWaiting = false;
local SoulstoneCooldown = false;
local SoulstoneAdvice = false;
local SoulstoneTarget = "";

-- Variables utilis�es dans la gestion des d�mons
local DemonType = nil;
local DemonEnslaved = false;

-- Variables utilis�es pour l'anti-fear
...
-- Variables utilis�es pour les �changes de pierre avec les joueurs
...
-- Variable contenant les derniers messages invoqu�s
local PetMess = 0
local SteedMess = 0
local RezMess = 0
local TPMess = 0

-- Permet la gestion des tooltips dans Necrosis (sans la frame des pi�ces de monnaie)
...
-- Fonction appliqu�e au chargement
function Necrosis_OnLoad()
	
	-- Permet de rep�rer les sorts lanc�s
	...
	-- Enregistrement des �v�nements intercept�s par Necrosis
...
-- Fonction appliqu�e une fois les param�tres des mods charg�s
function Necrosis_LoadVariables()
	...
	-- D�tection du type de d�mon pr�sent � la connexion
...
-- Fonction lanc�e � la mise � jour de l'interface (main) -- toutes les 0,1 secondes environ
function Necrosis_OnUpdate()

	-- La fonction n'est utilis�e que si Necrosis est initialis� et le joueur un D�moniste --
	...
	----------------------------------------------------------
	-- Gestion des sorts du D�moniste
	----------------------------------------------------------
	
	-- Gestion du menu d'invocation des d�mons
	if PetShow then
	...
	-- On met � jour les boutons toutes les secondes
...
-- Fonction lanc�e selon l'�v�nement intercept�
function Necrosis_OnEvent(event)
	...
	-- Traditionnel test : Le joueur est-il bien D�moniste ?
	-- Le jeu est-il bine charg� ?
	if (not Loaded) or (not Necrosis_In) or UnitClass("player") ~= NECROSIS_UNIT_WARLOCK then 
		return;
	end

	-- Si le contenu des sacs a chang�, on v�rifie que les Fragments d'�me sont toujours dans le bon sac
	...
	-- Quand le d�moniste commence � incanter un sort, on intercepte le nom de celui-ci
	...
	-- Quand le d�moniste stoppe son incantation, on relache le nom de celui-ci
	...
	-- Si le D�moniste apprend un nouveau sort / rang de sort, on r�cup�re la nouvelle liste des sorts
	-- Si le D�moniste apprend un nouveau sort de buff ou d'invocation, on recr�e les boutons
	elseif (event == "LEARNED_SPELL_IN_TAB") then
		Necrosis_SpellSetup();
		Necrosis_CreateMenu();
		Necrosis_ButtonSetup();
	
	-- A la fin du combat, on arr�te de signaler le Cr�puscule
	-- On enl�ve les timers de sorts ainsi que les noms des mobs
	...
	-- Quand le d�moniste change de d�mon
...
-- Fonction appliqu�e � chaque �cran de chargement
-- Quand on sort d'une zone, on arr�te de surveiller les envents
...
-- event : UNIT_PET
-- Permet de timer les asservissements, ainsi que de pr�venir pour les ruptures d'asservissement
-- Change �galement le nom du pet au remplacement de celui-ci
function Necrosis_ChangeDemon()
	-- Si le nouveau d�mon est un d�mon asservi, on place un timer de 5 minutes
	if (Necrosis_UnitHasEffect("pet", NECROSIS_SPELL_TABLE[10].Name)) then
		...
		-- Quand le d�mon asservi est perdu, on retire le Timer et on pr�vient le D�moniste
	...
	-- Si le d�mon n'est pas asservi on d�finit son titre, et on met � jour son nom dans Necrosis
	DemonType = UnitCreatureFamily("pet");
	for i = 1, 4, 1 do
...
-- Permet de g�rer les effets apparaissants et disparaissants sur le d�moniste
-- Bas� sur le CombatLog
function Necrosis_SelfEffect(action)
	if action == "BUFF" then
		-- Insertion d'un timer quand le D�moniste subit "Sacrifice"
		...
		-- Changement du bouton de monture quand le D�moniste chevauche
		...
		-- Changement du bouton de la domination corrompue si celle-ci est activ�e + Timer de cooldown
		...
		-- Changement du bouton de la mal�diction amplifi�e si celle-ci est activ�e + Timer de cooldown
		...
		-- Changement du bouton de monture quand le D�moniste est d�mont�
		if string.find(arg1, NECROSIS_SPELL_TABLE[1].Name) or  string.find(arg1, NECROSIS_SPELL_TABLE[2].Name) then
		...
		-- Changement du bouton de Domination quand le D�moniste n'est plus sous son emprise
		if  string.find(arg1, NECROSIS_SPELL_TABLE[15].Name) and NECROSIS_SPELL_TABLE[15].ID ~= nil then
			DominationUp = false;
			NecrosisPetMenu1:SetNormalTexture("Interface\\Addons\\Necrosis\\UI\\Domination-01");
		end
		-- Changement du bouton de la mal�diction amplifi�e quand le D�moniste n'est plus sous son emprise
		if  string.find(arg1, NECROSIS_SPELL_TABLE[42].Name) and NECROSIS_SPELL_TABLE[42].ID ~= nil then
			AmplifyUp = false;
...
-- Permet de g�rer tout ce qui touche aux sorts une fois leur incantation r�ussie
function Necrosis_SpellManagement()
	local SortActif = false;
	if (SpellCastName) then
		-- Si le sort lanc� � �t� une R�surrection de Pierre d'�me, on place un timer
		if (SpellCastName == NECROSIS_SPELL_TABLE[11].Name) then
			if SpellTargetName == UnitName("player") then
				SpellTargetName = "";
			end
			-- Si les messages sont actifs et que la pierre est pos�e sur un joueur cibl�, hop, message !
		...
		-- Si le sort �tait un rituel d'invocation, alors on �crit une phrase a caract�re informatif aux joueurs
		...
		-- Pour les autres sorts cast�s, tentative de timer si valable
			...
			for spell=1, table.getn(NECROSIS_SPELL_TABLE), 1 do
				if SpellCastName == NECROSIS_SPELL_TABLE[spell].Name and not (spell == 10) then
					-- Si le timer existe d�j� sur la cible, on le met � jour
					for thisspell=1, table.getn(SpellTimer), 1 do
						...
						-- Si c'est un fear, on supprime le timer du fear pr�c�dent
					...
					-- Si le timer est une mal�diction, on enl�ve la pr�c�dente mal�diction sur la cible
					if (NECROSIS_SPELL_TABLE[spell].Type == 4) or (spell == 16) then
						for thisspell=1, table.getn(SpellTimer), 1 do
							-- Mais on garde le cooldown de la mal�diction funeste
...
-- Fonction permettant le d�placement d'�l�ments de Necrosis sur l'�cran
function Necrosis_OnDragStart(button)
	if (button == "NecrosisIcon") then GameTooltip:Hide(); end
	button:StartMoving();
end

-- Fonction arr�tant le d�placement d'�l�ments de Necrosis sur l'�cran
...
-- Fonction g�rant les bulles d'aide
function Necrosis_BuildTooltip(button, type, anchor)

	-- Si l'affichage des bulles d'aide est d�sactiv�, Bye bye !
	if not NecrosisConfig.NecrosisToolTip then
		return;
	end
	
	-- On regarde si la domination corrompue, le gardien de l'ombre ou l'amplification de mal�diction sont up (pour tooltips)
	...
	if (type == "Main") then
		...
		-- On v�rifie si une pierre de sort n'est pas �quip�e
	...
	-- ..... pour les autres buffs et d�mons, le co�t en mana...
...
function Necrosis_UpdateIcons()
	...
	-- On se renseigne pour savoir si une pierre d'�me a �t� utilis�e --> v�rification dans les timers
	...
	-- Si la Pierre n'a pas �t� utilis�e, et qu'il n'y a pas de pierre en inventaire -> Mode 1
	...
	-- Si la Pierre n'a pas �t� utilis�e, mais qu'il y a une pierre en inventaire
	...
	-- Si la Pierre a �t� utilis�e mais qu'il n'y a pas de pierre en inventaire --> Mode 3
	...
	-- Si la Pierre a �t� utilis�e et qu'il y a une pierre en inventaire
	...
	-- Si cooldown de la mal�diction amplifi�e on grise
...
-- Fonction qui fait l'inventaire des �l�ments utilis�s en d�monologie : Pierres, Fragments, Composants d'invocation
function Necrosis_BagExplore()
	...
	for container=0, 4, 1 do
		...
		for slot=1, GetContainerNumSlots(container), 1 do
			...
			-- Si le sac est le sac d�fini pour les fragments
			-- hop la valeur du Tableau qui repr�sente le slot du Sac = nil (pas de Shard)
			...
			if itemName then
				...
				-- M�me chose pour une pierre de soin
				...
				-- On note aussi la pr�sence ou non des objets "main gauche"
	...
	-- S'il y a plus de fragment que d'emplacements dans le sac d�fini, on affiche un message d'avertissement
...
function Necrosis_SoulshardSwitch(type)
	...
	-- Apr�s avoir tout d�placer, il faut retrouver les emplacements des pierres, etc, etc...
	Necrosis_BagExplore();
end

-- Pendant le d�placement des fragments, il faut trouver un nouvel emplacement aux objets d�plac�s :)
function Necrosis_FindSlot(shardIndex, shardSlot)
	...
	-- Destruction des fragments en sur-nombre si l'option est activ�e
...
function Necrosis_SpellSetup()
	...
	-- On va parcourir tous les sorts possed�s par le D�moniste
	while true do
		...
		-- Pour les sorts avec des rangs num�rot�s, on compare pour chaque sort les rangs 1 � 1
		-- Le rang sup�rieur est conserv�
		if (string.find(subSpellName, NECROSIS_TRANSLATION.Rank)) then
			...
			-- Les plus grands rangs de chacun des sorts � rang num�rot�s sont ins�r�s dans la table
			if (not found) then
		...
		-- Test du Rang de la d�tection d'invisibilit�
		...
		-- Les pierres n'ont pas de rang num�rot�, l'attribut de rang fait partie du nom du sort
		...
		for stoneID=1, table.getn(StoneType), 1 do
			...
			-- d�j� assign� un rang maximum � cette derni�re
	...
	-- On ins�re dans la table les pierres avec le plus grand rang
	for stoneID=1, table.getn(StoneType), 1 do
	...
	-- On met � jour la dur�e de chaque sort en fonction de son rang
	...
	-- Insertion du plus haut rang de d�tection d'invisibilit� connu
	if Invisible >= 1 then
...
-- L'�clat doit avoir le rang le plus haut
...
-- Permet de reconnaitre quand le joueur gagne Cr�puscule / Transe de l'ombre
...
-- Fonction pour g�rer les actions effectu�es par Necrosis au clic sur un bouton
function Necrosis_UseItem(type,button)
	...
	-- s'il y en a une dans l'inventaire, et si c'�tait un click droit
	...
	-- Si on clique sur le bouton de la pierre d'�me
	-- On met � jour le bouton pour savoir quel est le mode
	if (type == "Soulstone") then
		Necrosis_UpdateIcons();
		-- Si le mode = 2 (une pierre dans l'inventaire, pas de pierre utilis�e)
		-- alors on l'utilise
		if (SoulstoneMode == 2) then
			-- Si un joueur est cibl�, sur le joueur (avec message d'alerte)
			-- Si un joueur n'est pas cibl�, sur le D�moniste (sans message)
			...
			-- Maintenant que l'on cr�e un timer sur la session, nous ne sortons plus d'un RL
			NecrosisRL = false;
			-- Et hop, on remet � jour l'affichage des boutons :)
			Necrosis_UpdateIcons();
		-- si il n'y a pas de pierre d'�me dans l'inventaire, alors on cr�e la pierre d'�me de rang le plus grand :)
	...
	elseif (type == "Healthstone") then
		...
		if HealthstoneOnHand then
			-- Dans ce cas si un pj alli� est s�lectionn�, on lui donne la pierre
			-- Sinon, on l'utilise
		...
		-- soit il n'y en a pas dans l'inventaire, et la pierre de plus haut rang est cr��e
		else
...
-- Fonction permettant de permutter un objet main-gauche �quip� avec un objet main-gauche de l'inventaire
function Necrosis_SwitchOffHand(type)
	if (type == "Spellstone") then
		if SpellstoneMode == 3 then
			...
			PickupInventoryItem(17);
			-- Le switch avec une pierre de sort implique un cooldown port� au timer
...
-- A chaque changement du livre des sorts, au d�marrage du mod, ainsi qu'au changement de sens du menu on reconstruit les menus des sorts
function Necrosis_CreateMenu()
	...
	-- Si le sacrifice d�moniaque existe, on affiche le bouton dans le menu des pets
	if NECROSIS_SPELL_TABLE[44].ID then
	...
	-- Maintenant que tous les boutons de pet sont plac�s les uns � c�t� des autres (hors de l'�cran), on affiche les disponibles
	for i = 1, table.getn(PetMenuCreate), 1 do
		ShowUIPanel(PetMenuCreate[i]);
	end

	-- Si l'Armure D�moniaque existe, on affiche le bouton dans le menu des buffs
	...
	-- Si la d�tection de l'invisibilit� existe, on affiche le bouton dans le menu des buffs (au plus haut rang)
	if NECROSIS_SPELL_TABLE[33].ID then
	...
	-- Si le Radar � d�mon existe, on affiche le bouton dans le menu des buffs
	...
	-- Maintenant que tous les boutons de buff sont plac�s les uns � c�t� des autres (hors de l'�cran), on affiche les disponibles
	for i = 1, table.getn(BuffMenuCreate), 1 do
		ShowUIPanel(BuffMenuCreate[i]);
	end

	-- Si la Mal�diction amplifi�e existe, on affiche le bouton dans le menu des curses
	if NECROSIS_SPELL_TABLE[42].ID then
	...
	-- Si la Mal�diction de faiblesse existe, on affiche le bouton dans le menu des curses
	if NECROSIS_SPELL_TABLE[23].ID then
	...
	-- Si la Mal�diction d'agonie existe, on affiche le bouton dans le menu des curses
	if NECROSIS_SPELL_TABLE[22].ID then
	...
	-- Si la Mal�diction de t�m�rit� existe, on affiche le bouton dans le menu des curses (au plus haut rang)
	if NECROSIS_SPELL_TABLE[24].ID then
	...
	-- Si la Mal�diction des languages existe, on affiche le bouton dans le menu des curses
	if NECROSIS_SPELL_TABLE[25].ID then
	...
	-- Si la Mal�diction de fatigue existe, on affiche le bouton dans le menu des curses
	if NECROSIS_SPELL_TABLE[40].ID then
	...
	-- Si la Mal�diction des �l�ments existe, on affiche le bouton dans le menu des curses
	if NECROSIS_SPELL_TABLE[26].ID then
	...
	-- Si la Mal�diction de l'ombre, on affiche le bouton dans le menu des curses
	if NECROSIS_SPELL_TABLE[27].ID then
	...
	-- Si la Mal�diction funeste existe, on affiche le bouton dans le menu des curses
	if NECROSIS_SPELL_TABLE[16].ID then
...
function Necrosis_BuffCast(type)
	...
	-- Si le D�moniste poss�de la peau de d�mon mais pas l'armure d�moniaque
...
-- Gestion des casts du menu des d�mons
...
-- Fonction permettant l'affichage des diff�rentes pages du livre des configurations
